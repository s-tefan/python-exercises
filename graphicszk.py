# graphicszk.py
"""
En modifierad version av John Zelles graphics.py
Modifierad av Stefan Karlsson
--------------------------------------------------------------------
...
# class Plot, Stefan Karlsson
class Plot(GraphicsObject):
   ...
   def __repr__(self):
      return "Plot({})".format(str(self.points))

   def clone(self):
      other = Plot(self.points)
      other.config = self.config.copy()
      return other
...
def plottest():
   ...
   p=Plot([Point(0,0),Point(5,4),Point(6,8),Point(9,5)])
   print(p)
   p.draw(win)
