class Quaternion :
    ...
    def __add__(self, a):
        if isinstance(a, Quaternion):
            return Quaternion(self.real + a.real, 
                tuple(self.imag[k] + a.imag[k] for k in range(3)))
        else:
            return Quaternion(self.real + a.real, self.imag )

    
    def __neg__(self):
        return Quaternion(-self.real,
            tuple(-k for k in self.imag)
        )
    ...
    def __mul__(self, a):
        if isinstance(a, Quaternion):
            ...
            imlist = [0,0,0]
            for k in range(3):
                imlist[k] = self.real*v[k] + a.real*u[k] +\
                    u[(k+1)%3]*v[(k-1)%3] - v[(k+1)%3]*u[(k-1)%3]
